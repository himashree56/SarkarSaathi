...
def _extract_children(text):
    m = re.search(r'(\d+)\s*(?:बच्चे|बच्चों|children|child|kids)', text, re.IGNORECASE)
...
def _infer_from_context(message, last_bot_msg, current_profile):
    ...
    # [1] Marital Status
    if any(k in last_bot_msg for k in ["married", "marriage", "शादी", "marital"]):
        if is_yes: inferred["marital_status"] = "married"
        elif is_no or msg == "single": inferred["marital_status"] = "unmarried"
...
def lambda_handler(event, context):
    ...
    if path.endswith("/query") and method == "POST":
        ...
        target_lang = body.get("language") or body.get("lang")
        if not target_lang and session.get("profile"):
            target_lang = session["profile"].get("language")
        ...
        if target_lang.lower() in LANG_MAP: target_lang = LANG_MAP[target_lang.lower()]
        ...
        # Multilingual Translation (Full list to ensure UI consistency)
        if target_lang != "en":
            ...
            if isinstance(known_profile, dict):
                for field in ["age","gender","marital_status","occupation","income_level",
                    ...
                    if profile.get(field) is None:
                        # Ensure we don't overwrite a factual value with an empty string from the frontend
...
def _batch_translate(items, target_lang):
    ...
    def translate_chunk(chunk, start_idx):
        ...
        combined_text = "\n---\n".join(to_translate)
        prompt = f"Translate the following Indian government scheme data into {lang_full}. Translate EVERYTHING accurately. Maintain the NAME_N:, BENEFIT_N:, REASON_N:, APPLY_N:, OFFICE_N:, DOCS_N:, and STEPS_N: prefixes exactly. Return ONLY the translated text:\n\n{combined_text}"
        ...
        try:
            ...
            print(f"[translate_chunk] failed for idx {start_idx}: {e}")
            return ""

    # Execute translation chunks in parallel (max 5 threads to avoid Bedrock throttling)
    ...
    for translated_blob in results:
        if not translated_blob: continue
        for line in translated_blob.split("\n"):
            clean_line = line.strip().replace("**", "")
            if ": " not in clean_line: continue
            
            try:
                prefix, content = clean_line.split(": ", 1)
                prefix = prefix.strip()
                idx_m = re.search(r'\d+', prefix)
                if not idx_m: continue
                idx = int(idx_m.group())
                if idx >= len(items): continue
                
                if prefix.startswith("NAME_"): items[idx]["name"] = content; items[idx]["name_en"] = content
                elif prefix.startswith("BENEFIT_"): items[idx]["benefit_description"] = content
                elif prefix.startswith("REASON_"): items[idx]["eligibility_reason"] = content
                elif prefix.startswith("APPLY_"): items[idx]["how_to_apply"] = content
                elif prefix.startswith("OFFICE_"): items[idx]["nodal_office"] = content
                elif prefix.startswith("DOCS_"): items[idx]["required_documents"] = [x.strip() for x in content.split("|") if x.strip()]
                elif prefix.startswith("STEPS_"): items[idx]["application_steps"] = [x.strip() for x in content.split("|") if x.strip()]
            except: pass
...
def _nova_summarize(profile, schemes, lang):
    ...
    for f, label in [("age","Age"),("gender","Gender"),("occupation","Occupation"),
        ...
        val = profile.get(f)
        if val: pparts.append(f"{label}: {val}")
...
def handle_tts(event, body):
    ...
    if lang != "en":
        ...
        try:
            # Explicit instruction for phonetic transliteration to LATIN script
            prompt = f"""Transliterate the following {lang_name} text into Romanized / LATIN English script (phonetic English). 
            Rules:
            - STRICTLY ONLY return the transliterated text in English/Latin characters.
            - DO NOT return the original {lang_name} script characters.
            - Do not translate the meaning, just convert the characters to English phonetics.
            - Example: "સંત્વના ગ્રાન્ટ યોજના" -> "Santvana Grant Yojana"
            ...
            # Switch to Sonnet for higher transliteration accuracy
            resp = bedrock.invoke_model(
                modelId="anthropic.claude-3-5-sonnet-20240620-v1:0",
                body=json.dumps(payload)
            )
            raw_body = resp['body'].read().decode()
            print(f"[tts] Bedrock raw response: {raw_body}")
            result = json.loads(raw_body)
            romanized_text = result['content'][0]['text'].strip()
            print(f"[tts] Romanized ({lang}): {romanized_text[:50]}...")
        except Exception as e:
